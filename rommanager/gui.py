...
class ROMManagerGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(hdr, text=_tr("title_main"), style='Header.TLabel').pack(side=tk.LEFT)
        ttk.Button(hdr, text=_tr("new_session"), command=self._new_session).pack(side=tk.RIGHT)
    ...
    def _persist_session(self):
        save_session(
            self.multi_matcher.get_dat_list(),
            self.identified,
            self.unidentified,
            {
                "scan_folder": self.scan_path_var.get(),
        ...
        if dat_infos:
            for di in dat_infos:
                try:
                    ...
                    pass
            self._refresh_dats()
        self.identified = data.get("identified", [])
        self.unidentified = data.get("unidentified", [])
        self.scanned_files = self.identified + self.unidentified
        ui = data.get("ui", {})
        if ui.get("scan_folder"):
            self.scan_path_var.set(ui["scan_folder"])
        if ui.get("output"):
            self.output_var.set(ui["output"])
        if ui.get("strategy"):
            self.strategy_var.set(ui["strategy"])
        if ui.get("action"):
            self.action_var.set(ui["action"])
        self._update_stats()
        self._refill_id()
        self._refill_un()
        self._refresh_missing()
    ...
    def _new_session(self):
        ans = messagebox.askyesnocancel(_tr("confirm"), _tr("new_session_confirm_save"))
        if ans is None:
            return
        if ans:
            self._persist_session()
